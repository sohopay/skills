...
Pass `idempotency_key` (UUID v4) on create/revoke when the harness cannot set HTTP headers — see `{SKILL:sohopay-idempotency}`.
...
- Spend and pay: `curl -fsSL {SKILL:sohopay-spend}`
- Setup index: `curl -fsSL {SKILL:sohopay-setup}`
