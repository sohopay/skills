...
Payment itself is the x402 merchant header rail (`PAYMENT-SIGNATURE` / `X-PAYMENT`), not an MCP settle tool — see `{SKILL:sohopay-x402}`. Borrower wallet repay is `{SKILL:sohopay-repay}` — leave `create_repayment` / `execute_repayment` prepare-only.
