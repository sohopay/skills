...
Required for the default hosted staging path: **network access** (ability to fetch skill docs and reach `staging.mcp.sohopay.xyz` / `staging.api.sohopay.xyz`). **Node.js and npm are not required** and must not be asked for or installed as part of normal setup — including staging.
