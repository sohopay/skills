...
7. **Protocol V2** — during **onboarding** (not first pay): `register_borrower` (store `operational_agent_id`) → wallet proof → token (include `credit:facility:accept`) → Ed25519 keygen + `register_agent_workload_key` → `authorize_agent` (grant must be ACTIVE). Later `prepare_x402_payment` may return `VOUCHER_ISSUED` → agent signs voucher / `PAYMENT-SIGNATURE`. On `X402_AGENT_KEY_NOT_REGISTERED` / `AGENT_AUTHORIZATION_REQUIRED`, onboard was skipped — recover, then retry the same idempotency key. Do not fall back to custodial `sign_transaction` under V2. Sign recipe + fixed key path: `x402-credit-pay.md`. Recovery: `x402-credit-pay.md` § Recovery if onboard was skipped.
