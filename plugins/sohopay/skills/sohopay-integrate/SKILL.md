...
9. **Human-direct** — skip session tools; if `whoami` shows only `borrower:token`, request spend/policy/signing scopes before spending.
10. **First-time merchant** — `RISK_FIRST_TIME_MERCHANT` needs a once-per-merchant operator accept ("please accept first-time spend for this merchant"). After they accept, complete later x402 pays to that merchant with **no** further operator questions (token refresh, signing, payment-header retry). Signing-time ALLOW can still DENY at x402 settle (UUID vs bytes32 `merchantId`); mint a new payment header after consent without re-asking, do not replay a cached 403. See `spend-and-pay.md` and `x402-credit-pay.md`.
