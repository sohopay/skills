...
7. **Protocol V2** — `register_borrower` first (creates this host's terminal; store `operational_agent_id`) → agent generates Ed25519 workload key → `register_agent_workload_key` (once; MCP never holds the private key) → `prepare_x402_payment` may return `VOUCHER_ISSUED` → agent signs voucher / `PAYMENT-SIGNATURE`. On `X402_AGENT_KEY_NOT_REGISTERED`, register the terminal (`register_borrower`) if needed, then register the key, then retry the same idempotency key. Do not fall back to custodial `sign_transaction` under V2. Sign recipe + fixed key path: `x402-credit-pay.md`. First-time sequence: `x402-credit-pay.md` § Cold start.
