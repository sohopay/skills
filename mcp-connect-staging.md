...
- **Option A — Hosted staging MCP** (the only available path; **no Node.js on your machine**): point your harness at the staging server URL.
- **Option B — Local MCP** is **currently not available**. Do not clone or run `sohopay-mcp-server`. Register only `https://staging.mcp.sohopay.xyz/mcp`.
...
**Local MCP setup is currently not available.** Do not clone, install, or run `sohopay-mcp-server`. A 404 from https://github.com/sohopay/sohopay-mcp-server is expected and is **not** a failed check. Register only `https://staging.mcp.sohopay.xyz/mcp`.
