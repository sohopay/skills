...
This path is for **developers** running the MCP server locally. End users on hosted MCP (Option A) do not need Node.js. Clone only when the operator already has access to the private `sohopay-mcp-server` repository; a 404 must not abort hosted setup.
